...
export const SUITES = {
  core: {
    ...
    triggers: [
      ...
      /^skill\/(SKILL\.src\.md|agents\/|reference\/|scripts\/(cleanup-deprecated|concept-seed|context|context-signals|critique-storage|design-parser|hook|impeccable-paths|is-generated|lib\/(composition-catalog|concept-catalog|provider|surface-briefs|target-slug)|pin|surface-brief))/,
      /^README(\.npm)?\.md$/,
      /^cli\/bin\//,
      /^tests\/(build|cleanup-deprecated|cli-ignores|concept-seed|context|context-signals|critique-storage|design-parser|github-sheriff|hook|hook-build|impeccable-paths|openai-plugin|pin|skills-cli|surface-brief|target-args|test-suites|windows-path-fix|zip)\.test\.(js|mjs)$/,
      /^tests\/lib\//,
    ],
    commands: [
      {
